...
public class OrderController {
     ...
     */
    @PutMapping(value = "/{id}")
    public Result update(@RequestBody(required = true) Order order, @PathVariable(value = "id") String id) {
        //设置主键值
        order.setId(id);
     ...
     */
    @PostMapping
    public Result<Order> add(@RequestBody(required = true) Order order) {
        //调用OrderService实现添加Order
        String username = tockenDecode.getUserInfo().get("username");
        order.setUsername(username);
        orderService.add(order);
        return new Result<Order>(true, StatusCode.OK, "添加成功");
    }
